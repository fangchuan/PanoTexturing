...
#pragma omp critical(vpis)
  ...
  if (matrix_size != 0) {
    ...
    for (std::size_t j = 0; j < num_vertices; ++j) {
      ...
      float sum = 0.0f;
      for (it = weights.begin(); it != weights.end(); ++it) sum += it->second;
      if (sum < std::numeric_limits<float>::epsilon()) return false;
      for (it = weights.begin(); it != weights.end(); ++it) it->second /= sum;
...
#pragma omp critical
  ...
  //    std::endl; std::cout << "\t" << num_patches << " texture patches." <<
  //    std::endl;
}
      ...
      if (success) {
        ...
        if (pano_views.size() > 1) {
          ...
          for (int j = 0; j < subgraph.size(); ++j) {
            ...
            for (int i = 0; i < pano_views.size(); ++i) {
              math::Vec3f candidate_vertex =
                  vertices[mesh_faces[subgraph[j] * 3]];
              math::Vec3f pano_pose = pano_views[i].get_pos();
              float distance =
                  std::pow(std::fabs(pano_pose[0] - candidate_vertex[0]), 2) +
                  std::pow(std::fabs(pano_pose[1] - candidate_vertex[1]), 2) +
                  std::pow(std::fabs(pano_pose[2] - candidate_vertex[2]), 2);
              if (distance < min_distance &&
                  std::fabs(pano_pose[1] - candidate_vertex[1]) < 1) {
          ...
          minIndex = static_cast<int>(std::distance(
              all_min_index.begin(),
              max_element(all_min_index.begin(), all_min_index.end())));
        }
        ...
        byte_image = mve::image::crop(view_image, width, height, min_x, min_y,
                                      *math::Vec3uc(255, 0, 255));
        mve::FloatImage::Ptr image =
            mve::image::byte_to_float_image(byte_image);
