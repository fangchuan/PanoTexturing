...
struct TexturePatchCandidate {
  mvs_texturing::Rect<int> bounding_box;
...
generate_candidate(int label, TextureView const & texture_view,
    ...
    TexturePatchCandidate texture_patch_candidate =
        {mvs_texturing::Rect<int>(min_x, min_y, max_x, max_y),
    ...
    for (std::size_t i = 0; i < texture_views->size(); ++i) {
        ...
        for (it = candidates.begin(); it != candidates.end(); ++it) {
            for (sit = candidates.begin(); sit != candidates.end();) {
                mvs_texturing::Rect<int> bounding_box = sit->bounding_box;
...
generate_texture_patches(UniGraph const & graph, mve::TriangleMesh::ConstPtr mesh,
                         ...
                         std::vector<std::vector<VertexProjectionInfo> > * vertex_projection_infos,
                         std::vector<TexturePatch::Ptr> * texture_patches,
                         const TextureViews &pano_views,
                         std::vector<TexturePatch::Ptr> * unseen_texture_patches) {
...

  {
    ...
    for (std::size_t k = 0; k < subgraphs.size(); ++k) {
      std::vector<std::size_t> const & subgraph = subgraphs[k];
      ...
      if (success) {
        ...
        if(pano_views.size()>1){
          // select pano_view
          std::vector<size_t> all_min_index;
          all_min_index.resize(pano_views.size());
          for (int i = 0; i < pano_views.size(); ++i) {
            all_min_index[i] = 0;
          }
          for (int j = 0; j < subgraph.size(); ++j) {
            ...
            for (int i = 0; i < pano_views.size(); ++i) {
              math::Vec3f candidate_vertex = vertices[mesh_faces[subgraph[j] * 3]];
              math::Vec3f pano_pose = pano_views[i].get_pos();
              float distance = std::pow(std::fabs(pano_pose[0] - candidate_vertex[0]), 2) +
                  std::pow(std::fabs(pano_pose[1] - candidate_vertex[1]), 2) +
                  std::pow(std::fabs(pano_pose[2] - candidate_vertex[2]), 2);
              if (distance < min_distance && std::fabs(pano_pose[1] - candidate_vertex[1])<1) {
                min_distance = distance;
                min_index = i;
              }
            }
            all_min_index[min_index] = all_min_index[min_index] + 1;
          }
          minIndex =  static_cast<int>(std::distance(all_min_index.begin(), max_element(all_min_index.begin(), all_min_index.end())));
        }

        auto texture_view = pano_views.at(minIndex);
        int min_x = texture_view.get_width(), min_y = texture_view.get_height();
        int max_x = 0, max_y = 0;
        std::vector<math::Vec2f> texcoords;
        ...
        for (std::size_t i = 0; i < subgraph.size(); ++i) {
          for (std::size_t j = 0; j < 3; ++j) {
            ...
            math::Vec2f pixel = texture_view.get_pano_pixel_coords(vertex);
            texcoords.push_back(pixel);
            min_x = std::min(static_cast<int>(std::floor(pixel[0])), min_x);
            min_y = std::min(static_cast<int>(std::floor(pixel[1])), min_y);
            max_x = std::max(static_cast<int>(std::ceil(pixel[0])), max_x);
        ...
        unseen_texture_patches->push_back(texture_patch);
