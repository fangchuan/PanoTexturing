...
TextureView::TextureView(std::size_t id, mve::CameraInfo const & camera, const cv::Mat & cv_image): id(id), cv_mat_(cv_image){

  width = cv_image.size().width;
  height = cv_image.size().height;
...
void
TextureView::AddValidMask(const cv::Mat &mask) {
  mask_ = mask;
}
  ...
  if (!mask_.empty()){
    for (int i = 0; i < height; ++i) {
      for (int j = 0; j < width; ++j) {
        int mask_value = mask_.at<uchar>(i, j);
        if (mask_value<100) validity_mask[j + i * width] = false;
      }
    }
  }
}
...
TextureView::load_image(void) {
    ...
    if (!cv_mat_.empty()){
      image = ConvertCVMatToByteImage(cv_mat_);
    }
    if(image != NULL) return;
    image = mve::image::load_file(image_file);
}

mve::ByteImage::Ptr TextureView::ConvertCVMatToByteImage(const cv::Mat &cv_mat){

  const int width = cv_mat.size().width;
  const int height = cv_mat.size().height;
  mve::ByteImage::Ptr byt_img = mve::ByteImage::create(width, height, 3);

  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
      byt_img->at(x,y,2) = cv_mat.at<cv::Vec3b>(y,x)(0);
      byt_img->at(x,y,1) = cv_mat.at<cv::Vec3b>(y,x)(1);
      byt_img->at(x,y,0) = cv_mat.at<cv::Vec3b>(y,x)(2);
    }
  }
